...
from handlers.client_model.errors import Error as bError
...
class RegisterUploadResult(BaseModel):
    ...
    """
    RelUrlOfServerPath: Union[str, None] = Field(
        description="Đường dẫn tương đối để xem hoặc tãi nội dung (không có url gốc)")
    """
    ...
    """
    RelUrlThumb: Union[str, None] = Field(
        description="Đường tương đối đến ảnh Thumb\n<br><host api url>+RelUrlThumb=UrlThumb")
    """
...
class RegisterUploadInfoResult(BaseModel):
    ...
    Error: Union[bError, None] = Field(description="nếu không thành công lỗi ở đây")
